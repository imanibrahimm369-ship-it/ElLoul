    ...
    const App = {
        ...
        cacheElements() {
            this.elements = {
                ...
                cartSubtotalPrice: document.getElementById('cart-subtotal-price'), // (جديد)
                ...
                if (firebaseUser) {
                    ...
                    this.state.user.unsubscribe = onSnapshot(userDocRef, (userDoc) => {
                        if (userDoc.exists() && userDoc.data().status === 'approved') {
                            ...
                            console.warn(`User ${firebaseUser.uid} is authenticated but no document exists in Firestore.`);
                        }
                    }, (error) => {
        ...
        async handleSignup(e) { 
            ...
            try { 
                ...
                try {
                    ...
                    // (جديد) إرسال إشعار Telegram
                    const telegramMessage = `✅ تسجيل مستخدم جديد!\nالاسم: ${name}\nالإيميل: ${email}\nالهاتف: ${phone || 'لا يوجد'}`;
        ...
        // (تعديل) استخدام أيقونة تحميل أفضل
        ...
        initInventory() {
            if (this.state.inventory.unsubscribeProducts) return; 
        ...
        // (تعديل) استخدام Notyf لرسالة الترحيب
        ...
        // (تعديل) دالة عرض المنتجات مع التبويبات والشارات والصور البديلة
        renderProductBatch(items) { 
            ...
            items.forEach((item, index) => { 
                ...
                // (جديد) تأخير الأنيميشن للتأثير المتتابع
                card.style.animationDelay = `${index * 50}ms`;
                card.classList.add('product-card-fade-in');

                // (جديد) الصورة البديلة باسم المنتج
                const placeholderText = encodeURIComponent(item['الصنف'] || 'منتج');
                const placeholderImg = `https://placehold.co/400x200/eef2ff/4f46e5?text=${placeholderText}&font=tajawal`;
                const imagePart = item.صورة 
                    ? `<img src="${item.صورة}" class="product-image product-image-btn" data-id="${item.id}" alt="${this.escapeHtml(item['الصنف'])}" loading="lazy" onerror="this.onerror=null;this.src='${placeholderImg}';">` 
                    : `<img src="${placeholderImg}" class="product-image product-image-btn" data-id="${item.id}" alt="صورة بديلة لـ ${this.escapeHtml(item['الصنف'])}" loading="lazy">`;
                ...
                if (quantity === -1) {
                    ...
                    quantityBadge = `<div class="product-quantity-badge quantity-out" title="نفدت الكمية"><i class="fas fa-times"></i> نفدت</div>`;
                } else if (quantity <= 10) {
                    quantityBadge = `<div class="product-quantity-badge quantity-low" title="كمية قليلة"><i class="fas fa-exclamation-triangle"></i> ${quantity}</div>`;
                } else {
                    quantityBadge = `<div class="product-quantity-badge quantity-high" title="كمية متاحة"><i class="fas fa-check"></i> ${quantity}</div>`;
                }

                const isOutOfStock = quantity === 0;
                
                // (جديد) تبويبات الأدمن مقابل المستخدم
                let footerHtml = '';
                const addToCartTab = `<button class="product-card-tab add-tab" data-id="${item.id}" ${isOutOfStock ? 'disabled' : ''}>
                                        ${isOutOfStock ? 'نفدت الكمية' : '<i class="fas fa-cart-plus mr-2"></i> إضافة للسلة'}
                                      </button>`;
                
                if (this.state.user.role === 'admin') {
                    const editTab = `<button class="product-card-tab edit-tab" data-id="${item.id}">
                                        <i class="fas fa-edit mr-2"></i> تعديل
                                     </button>`;
                    footerHtml = `<div class="product-card-footer admin-tabs">
                                    ${addToCartTab}
                                    ${editTab}
                                  </div>`;
                } else {
                ...
                card.innerHTML = `<div class="relative">
                                  ...
                                  <div class="p-4 flex flex-col flex-grow">
                                    ...
                                    <div class="mt-4 flex-grow">
                                        <p class="text-primary text-2xl font-black">${Number(item['السعر']).toFixed(2)} جم</p>
                                    </div>
                                  </div>
                                  ${footerHtml}`; // (تعديل) استخدام التبويبات الجديدة
        ...
        // (تعديل) منطق النقر على البطاقة لاستخدام التبويبات
        handleGridClick(e) { 
            const addTabBtn = e.target.closest('.add-tab');
            const editTabBtn = e.target.closest('.edit-tab');
            const productImage = e.target.closest('.product-image-btn');
            
            const id = addTabBtn?.dataset.id || editTabBtn?.dataset.id || productImage?.dataset.id;
            if (!id) return; 
            
            const product = this.state.inventory.fullInventory.find(p => p.id === id); 
            if (!product) return; 

            if (productImage) { this.showImageModal(product); return; } 
            if (addTabBtn) { this.addToCart(product); return; } 
            if (editTabBtn && this.state.user.role === 'admin') { this.showProductModal(product); return; } 
        },
        ...
        // (تعديل) نافذة إضافة/تعديل منتج مع زر الحذف
        async showProductModal(product = null) { 
            ...
            const { value: formValues, isConfirmed, isDenied } = await Swal.fire({ 
                ...
                showDenyButton: isEditing, // (جديد)
                denyButtonText: 'حذف المنتج', // (جديد)
                denyButtonColor: '#dc2626', // (جديد)
            ...
            // (جديد) التعامل مع زر الحذف
            if (isDenied) {
                this.deleteProduct(product); // استدعاء دالة الحذف (التي تحتوي على تأكيد خاص بها)
                return;
            }
        ...
        // --- دوال لوحة الأدمن (مع تأثيرات بصرية) ---
        showAdminPanel() {
        ...
        // (تعديل) دالة عرض السلة بالتصميم الجديد
        renderCartPanel() { 
            const totalItems = this.state.cart.reduce((sum, item) => sum + item.quantity, 0); 
            this.elements.cartPanelCount.textContent = totalItems; 
            if (this.state.cart.length === 0) { 
                this.elements.cartItemsContainer.innerHTML = ''; 
                this.elements.cartEmptyMsg.style.display = 'block'; 
                this.elements.checkoutBtn.disabled = true; 
                this.elements.cartSubtotalPrice.textContent = '0.00 جم';
                this.elements.cartTotalPrice.textContent = '0.00 جم'; 
                return; 
            } 
            this.elements.cartEmptyMsg.style.display = 'none'; 
            this.elements.cartItemsContainer.innerHTML = ''; 
            let totalPrice = 0; 
            this.state.cart.forEach(item => { 
                ...
                const placeholderImg = `https://placehold.co/80x80/eef2ff/4f46e5?text=${encodeURIComponent(item.name[0])}&font=tajawal`;
                itemElement.innerHTML = `
                    <img src="${item.image || placeholderImg}" alt="${this.escapeHtml(item.name)}" class="w-20 h-20 object-cover rounded-lg border flex-shrink-0" onerror="this.onerror=null;this.src='${placeholderImg}';">
                    <div class="flex-1">
        ...
        // (تعديل) إضافة إشعار Telegram عند إتمام الطلب
        async processCheckout() { 
            this.setLoading(this.elements.checkoutBtn, true, 'جاري معالجة الطلب...'); 
            let orderData; // (جديد) تعريف المتغير هنا
            try { 
                await runTransaction(db, async (transaction) => { 
                    const itemsToOrder = [...this.state.cart]; 
                    const productRefs = {}; 
                    const productDocs = {}; 
                    for (const item of itemsToOrder) { 
                        ...
                        if (!productDoc.exists()) { throw new Error(`المنتج ${item.name} لم يعد متاحاً.`); } 
                        productDocs[item.id] = productDoc.data(); 
                    } 
                    for (const item of itemsToOrder) { 
                        const productData = productDocs[item.id]; 
                        const currentStock = productData['الكمية'] !== undefined ? parseInt(productData['الكمية']) : -1; 
                        if (currentStock === -1) continue; 
                        if (currentStock < item.quantity) { throw new Error(`الكمية المتاحة من ${item.name} هي ${currentStock} فقط.`); } 
                    } 
                    ...
                    orderData = { 
                        ...
                        items: itemsToOrder.map(i => ({ id: i.id, name: i.name, quantity: i.quantity, price: i.price })), 
                        totalPrice: total, 
                        status: 'pending', 
                        createdAt: new Date() 
                    }; 
                    ...
                    for (const item of itemsToOrder) { 
                        const productRef = productRefs[item.id]; 
                        const productData = productDocs[item.id]; 
                        const currentStock = productData['الكمية'] !== undefined ? parseInt(productData['الكمية']) : -1; 
                        if (currentStock !== -1) { 
                            const newStock = currentStock - item.quantity; 
                            transaction.update(productRef, { "الكمية": newStock }); 
                        } 
                    } 
                }); 

                // (جديد) إرسال إشعار Telegram بعد نجاح العملية
                const itemsText = orderData.items.map(item => `  - ${item.name} (الكمية: ${item.quantity})`).join('\n');
                const telegramMessage = `📦 طلب جديد!\nمن: ${orderData.userName}\nهاتف: ${orderData.userPhone || 'لا يوجد'}\nالإجمالي: ${orderData.totalPrice.toFixed(2)} جم\n\nالطلبات:\n${itemsText}`;
                await this.sendTelegramMessage(telegramMessage);

                Swal.fire('تم!', 'تم إرسال طلبك بنجاح! سيتم التواصل معك قريباً.', 'success'); 
                this.state.cart = []; 
                this.updateCartUI(); 
                this.closeCartPanel(); 
            } catch (error) { 
                console.error("Checkout Error:", error); 
                Swal.fire('خطأ في الطلب', error.message, 'error'); 
            } finally { 
        ...
        /**
         * (جديد) إرسال رسالة إلى Telegram
         * -----------------------------------------------------------------
         * !! تحذير أمني خطير !!
         * هذا الكود يعرض 'botToken' الخاص بك في كود العميل (المتصفح).
         * يمكن لأي شخص سرقته والتحكم في البوت الخاص بك.
         * للاستخدام الفعلي، يجب نقل هذا المنطق إلى خادم آمن (مثل Google Cloud Function)
         * واستدعاء تلك الدالة من هنا، وليس وضع التوكن في المتصفح.
         * تم تنفيذ هذا الكود بهذه الطريقة بناءً على الطلب المباشر فقط.
         * -----------------------------------------------------------------
         */
        async sendTelegramMessage(text) {
            ...
            const payload = {
                ...
                text: text,
                parse_mode: 'Markdown' // يمكنك تغييره إلى 'HTML' إذا أردت
            };
